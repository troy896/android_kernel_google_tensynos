...
// Forward declaration: add_to_dl_tree() may fall back to the prio queue
static void insert_to_prio_queue(
		struct adios_data *ad, struct request *rq, bool pq_idx);
...
static void add_to_dl_tree(
	...
	if (!dlg || dlg->deadline != deadline) {
		...
		if (!dlg) {
			/* Under memory pressure, fall back to the priority queue
			 * so the request is still dispatched instead of lost. */
...
static void insert_to_prio_queue(struct adios_data *ad,
	...
	/* rd may be NULL if adios_prepare_request() failed to allocate it
	 * under memory pressure; such a request is dispatched unmanaged. */
	if (likely(rd)) {
...
static void insert_request_post_stability(struct blk_mq_hw_ctx *hctx,
	...
	/* rd alloc failed at prepare time: dispatch unmanaged via the prio
	 * queue so the request is not lost. */
	if (unlikely(!rd)) {
		insert_to_prio_queue(ad, rq, !at_head);
		return;
	}
...
static void insert_request_pre_stability(struct blk_mq_hw_ctx *hctx,
	...
	/* rd alloc failed at prepare time: dispatch unmanaged. */
	if (unlikely(!rd)) {
...
static void adios_completed_request(struct request *rq, u64 now) {
	...
	/* Unmanaged request (rd alloc failed at prepare time): nothing
	 * was accounted for it, so there is nothing to reconcile. */
	if (unlikely(!rd))
		return;
	...
	lct = ad->last_completed_time ?: rq->io_start_time_ns;
	...
	if (ad->is_rotational) {
		...
		if (ad->last_completed_pos > 0) {
			...
			/* __builtin_clzll(0) is undefined; a zero seek is sequential. */
			weight = seek_distance ?
				65 - __builtin_clzll(seek_distance) : 1;
		}
